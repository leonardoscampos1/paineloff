...
def carregar_vendas(data_inicial, data_final):
    query = f"""
        SELECT CODFILIAL, DTMOV, CODOPER, CODCLI, CODUSUR, CODPROD, QT, PUNIT AS PRECO_FINAL, DTCANCEL, DESCRICAO
        FROM PCMOV
...
# 📥 Carregar dados
# ============================
with st.spinner("🔄 Carregando dados do banco remoto..."):
...
if escolha == "Alud":
    ...
    with col1:
        st.markdown(f"<div style='text-align:center; font-size:28px; font-weight:bold;'>💰 Meta</div>"
                    f"<div style='text-align:center; font-size:36px; color:#16a34a;'>R$ {meta:,.2f}</div>"
                    .replace(",", "X").replace(".", ",").replace("X", "."), unsafe_allow_html=True)
    with col2:
        st.markdown(f"<div style='text-align:center; font-size:28px; font-weight:bold;'>💵 Valor Faturado</div>"
                    f"<div style='text-align:center; font-size:36px; color:#dc2626;'>R$ {valor_total:,.2f}</div>"
                    .replace(",", "X").replace(".", ",").replace("X", "."), unsafe_allow_html=True)
    with col3:
        st.markdown(f"<div style='text-align:center; font-size:28px; font-weight:bold;'>🎯 Faltam para Meta</div>"
                    f"<div style='text-align:center; font-size:36px; color:#dc2626;'>R$ {faltante:,.2f}</div>"
                    .replace(",", "X").replace(".", ",").replace("X", "."), unsafe_allow_html=True)
